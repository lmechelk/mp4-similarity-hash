...
def plot_roc(
    ...
    if isinstance(input_csvs, str) and os.path.isdir(input_csvs):
        files = sorted(Path(input_csvs).glob("*.csv"))
    else:
        files = [Path(p) for p in input_csvs]
    ...
    for csv_path in files:
        ...
        label    = df["ground_truth_source"].iloc[0]
